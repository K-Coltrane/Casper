...
for (const worker of workers) {
    ...
    worker.on("failed", (job, error) => {
        logger.error({ worker: worker.name, jobId: job?.id ?? "unknown", err: error }, "job failed");
    });
}
await Promise.all(env.TRADING_PAIRS.split(",")
    .map((symbol) => symbol.trim().toUpperCase())
    .filter((symbol) => symbol.length > 0)
    .map((symbol) => queues.marketQueue.add("fetch-market", { symbol }, {
